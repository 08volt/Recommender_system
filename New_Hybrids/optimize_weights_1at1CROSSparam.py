...
def optimize_weights_1at1CROSSparam(URM_train,recs, inits, fits,levels = 2,weights = None,size = 30, normalize_scores = False,validations = 4):
    ...
    for opt in optimization_order:
        trials = np.linspace(0.0001, 1, num=size+1, endpoint=True)
...
#             recommender.fit(fits,weights)
        ...
        weights = optimize_process(URM_train,weights,opt, trials,recs, inits, fits,validations, 0, levels, size)
    return weights

def optimize_process(URM_train, weights,index, trials,recs, inits, fits,validations = 3, l=0, levels=2, size=30):
    ...
    if best_t <= 0.0001:
        ...
        left_arr = np.linspace(left, best_t, num=size//2, endpoint=False)
        right_arr = np.linspace(best_t, right, num=size//2, endpoint=False)
        trials = np.concatenate((left_arr,right_arr))[1:]

    return optimize_process(URM_train, weights,index,trials,recs, inits, fits,validations, l+1, levels,size)
