...
class SearchAbstractClassCross(object):
    ...
    def __init__(self, recommender_class,
        ...
        super(SearchAbstractClassCross, self).__init__()
    ...
    def _evaluate_on_validation(self, current_fit_parameters):
        ...
        if self.URM_arr == None or len(self.URM_arr) == 0:
            URM_all = self.recommender_input_args.CONSTRUCTOR_POSITIONAL_ARGS[0]
            self.URM_arr = []
            for v in range(self.n_validations):
        ...
        results = []
        for urm in self.URM_arr:
            self.recommender_input_args.CONSTRUCTOR_POSITIONAL_ARGS[0] = urm["URM_train"]
            ...
            evaluator_validation = EvaluatorHoldout(urm["URM_validation"], cutoff_list=[10])
        ...
        return results, None,self._fit_model(current_fit_parameters) , 0.1, 0.1
...
#             recommender_instance.save_model(self.output_folder_path, file_name =self.output_file_name_root + "_best_model_last")
    ...
    def _objective_function(self, current_fit_parameters_dict):
        ...
        try:
            ...
            results, result_string, recommender_instance, train_time, evaluation_time = self._evaluate_on_validation(current_fit_parameters_dict)
            
            tot_map = 0
            for r in results:
                tot_map += r[self.metric_to_optimize]
            

            current_result = tot_map/len(results)
            result_dict = {}
            result_dict[self.metric_to_optimize] = current_result
            result_string = f"FKN RESULT {current_result}"
